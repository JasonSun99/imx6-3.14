...
static int imx6_pcie_start_link(struct pcie_port *pp)
{
	...
	printk("**imx6_pcie_start_link ret is 0x%x \n",ret);
	if (ret)
		goto out;
	...
	if (imx6_pcie->link_gen == 2) {
		...
		dev_info(pp->dev, "Link: Gen2 disabled pci-imx6.c line 505\n");
	}
	...
	printk("**imx6_pcie_start_link ret is 0x%x again\n",ret);
...
{
	int ret;
	printk(" *******imx6_pcie_host_init is here  \n");//
...
{
	int ret;
	printk(" *******imx6_add_pcie_port is here  \n");//
	...
	printk(" *******imx6_add_pcie_port is end  \n");//
	if (ret) {
...
static int __init imx6_pcie_probe(struct platform_device *pdev)
{
	...
	int ret;
	printk(" \n *******imx6_pcie_probe is here! \n");
	printk(" **sizeof(*imx6_pcie) is 0x%x  \n",sizeof(*imx6_pcie));
	//printf("imx6_pcie_probe is here*******! \n");
	...
	//printk(" **dbi_base->start is 0x%x  \n",dbi_base);
	printk(" **dbi_base->start is 0x%x  \n",dbi_base->start);//0x01ff-c000
	printk(" **dbi_base->end is 0x%x  \n",dbi_base->end);//0x01ff-ffff
	...
	if (IS_ENABLED(CONFIG_EP_MODE_IN_EP_RC_SYS)) {
		...
		ret = of_property_read_u32(pp->dev->of_node, "fsl,max-link-speed",
					   &imx6_pcie->link_gen);
	    printk(" **fsl,max-link-speed is 0x%x  \n",ret);//
       		if (ret)
